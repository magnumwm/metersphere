    ...
    public void append(ILoggingEvent event) {
        try {
            if (!event.getLevel().levelStr.equals(LogUtil.DEBUG)) {
                ...
                if (FixedCapacityMapHandler.fixedCapacityCache.containsKey(event.getTimeStamp())) {
                    FixedCapacityMapHandler.fixedCapacityCache.get(event.getTimeStamp()).append(message);
                } else {
                    FixedCapacityMapHandler.fixedCapacityCache.put(event.getTimeStamp(), message);
                }
            }
        } catch (Exception e) {
            LogUtil.error(e);
